...
<head>
  ...
  <title>1 Day to Go — Gen AI to Z!</title>
</head>
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; background-color: #0a0a0a; color: #ffffff; margin: 0; padding: 0;">
  <div style="max-width: 600px; margin: 0 auto; padding: 40px 20px;">
    ...
    <!-- Countdown Banner -->
    <div style="text-align: center; background: linear-gradient(135deg, rgba(239, 68, 68, 0.3), rgba(217, 70, 239, 0.3)); border: 1px solid rgba(239, 68, 68, 0.5); border-radius: 12px; padding: 20px; margin-bottom: 24px;">
      <p style="font-size: 40px; font-weight: 800; margin: 0; background: linear-gradient(to right, #f87171, #f0abfc); -webkit-background-clip: text; -webkit-text-fill-color: transparent;">1 DAY TO GO! 🔥</p>
      <p style="color: #fca5a5; font-size: 15px; margin: 8px 0 0 0; font-weight: 700;">TOMORROW — Tuesday, March 17!</p>
    </div>
    ...
    <div style="background: linear-gradient(to bottom, rgba(139, 92, 246, 0.1), transparent); border: 1px solid rgba(139, 92, 246, 0.3); border-radius: 12px; padding: 32px;">
      ...
      <p style="color: #d4d4d8; line-height: 1.7; margin: 0 0 20px 0;">
        It's happening <strong>TOMORROW!</strong> 🎉 <strong>Gen AI to Z</strong> is just <strong>1 day away</strong>. We can't wait to see you at the David M. Consunji Theater at UP Diliman!
      </p>
      ...
      <div style="text-align: center; background: linear-gradient(135deg, rgba(239, 68, 68, 0.2), rgba(239, 68, 68, 0.08)); border: 1px solid rgba(239, 68, 68, 0.4); border-radius: 8px; padding: 16px; margin-bottom: 24px;">
        <p style="color: #fca5a5; font-size: 16px; font-weight: 700; margin: 0;">
          🚨 Due to high demand, seats are on a first come, first served basis — come early!
        </p>
      </div>
    ...
    <div style="text-align: center; margin-top: 32px; padding: 24px; background: rgba(139, 92, 246, 0.06); border: 1px solid rgba(139, 92, 246, 0.15); border-radius: 12px;">
      ...
      <p style="color: #a1a1aa; font-size: 14px; line-height: 1.6; margin: 0 0 12px 0;">
        Share the event with your friends! Walk-ins are welcome but seats are limited.
      </p>
  ...
  try {
    ...
    if (testEmail) {
      ...
      const { data, error } = await resend.emails.send({
        ...
        subject: `[TEST ${new Date().toLocaleTimeString('en-PH', { hour: '2-digit', minute: '2-digit' })}] 1 Day to Go! 🔥 Gen AI to Z — TOMORROW, March 17!`,
        html: buildEmail(recipientName),
        tags: [
          { name: 'type', value: '1day-reminder-test' },
    ...
    for (let i = 0; i < registrants.length; i += BATCH_SIZE) {
      ...
      const emailPayloads = batch.map(registrant => ({
        ...
        to: registrant.email,
        replyTo: 'hello@vibecoders.ph',
        subject: '1 Day to Go! 🔥 Gen AI to Z — TOMORROW, March 17!',
        html: buildEmail(registrant.full_name),
        tags: [
          { name: 'type', value: '1day-reminder' },
          { name: 'event', value: 'gen-ai-to-z' },
        ],
      }));
